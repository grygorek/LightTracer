...
//[/header]
//[compile]
// Download the raytracetransform.cpp, geometry.h and teapot.geo file to a
// folder. Open a shell/terminal, and run the following command where the files
// are saved:
//
...
template <>
const Matrix44f Matrix44f::kIdentity = Matrix44f();
              ...
              const Options &options, const uint32_t &depth = 0)
{
  ...
  if (trace(orig, dir, objects, isect))
  {
    ...
    case kDiffuse:
    {
      // Light loop (loop over all lights in the scene and accumulate their
      // contribution)
...
template <class Time>
auto TimeDeltaMilisecs(Time start, Time end)
{
...
int main(int argc, char **argv)
{
  ...
  options.cameraToWorld = Matrix44f(
      -0.972776, 0, -0.231748, 0, -0.114956, 0.8683, 0.482536, 0, 0.201227,
      0.49604, -0.844661, 0, 6.696465, 22.721296, -30.097976, 1);

  TriangleMesh *mesh1 =
      loadPolyMeshFromFile("./geometry/backdrop.geo", Matrix44f::kIdentity);
  if (mesh1 != nullptr)
  {
    mesh1->type = kDiffuse;
  ...
  TriangleMesh *mesh3 =
      loadPolyMeshFromFile("./geometry/cylinder.geo", Matrix44f::kIdentity);
  if (mesh3 != nullptr)
  {
  ...
  TriangleMesh *mesh4 =
      loadPolyMeshFromFile("./geometry/pen.geo", Matrix44f::kIdentity);
  if (mesh4 != nullptr)
  {
    mesh4->type          = kDiffuse;
    mesh4->albedo        = 0.18;
